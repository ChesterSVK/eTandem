...
class StudyLanguagesInput extends React.Component {
  constructor(props) {
    ...
    const allLevel= TandemLanguageLevels.findAll().map(function (level) {return{_id: level._id, level: level.level} });
  ...
  render() {
    ...
    const languageMenu = this.state.allLanguages.map( (language) => {
      ...
      if ( allPickedLanguages != null) {
        const a = allPickedLanguages.filter(pickedLanguage => {pickedLanguage.langId == language._id});
        disabled = a.length > 0;
      }
      return <MenuItem
        key={language._id}
